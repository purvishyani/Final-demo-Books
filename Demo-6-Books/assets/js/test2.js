...
if (homePage) {
    ...
    function jsonAddSection(sectionNo) {
        ...
        // console.log(BooksData.sections);
        return section;
    ...
    saveModalData.addEventListener("submit", (event) => {
        ...
        if (inputFields.length === inputValues.length) {
            BooksData.sections.forEach(id => {
                for (let i = 0; i < inputFields.length; i++) {
                    if (id.sectionId !== (i + 1).toString()) {
                        jsonAddSection((i + 1).toString());
                    }
                }
            });
        ...
        storedItems.forEach(item => {
            if (item.bookId === selectedId) {
                ...
                item.sections.forEach(sec => {
                    ...
                    document.getElementById("view-modal-content").innerHTML += `
                    <li class="section-header"><b>Main Section: </b>${sec.sectionName}</li>
                    `
                    sec.subsections.forEach(sub => {
                        console.log("    --" + sub.subSectionName);
                        document.querySelector(".section-header").innerHTML += `
                            <ul >
                                <li class="sub-header"><b>Sub Section: </b>${sub.subSectionName}</li>
                            <ul>
                        `
                        sub.children.forEach(child => {
                            ...
                            <ul>
                                <li><b>Child Section: </b>${child.childName}</li>
                            </ul>
                            `
                        });
                    });
